...
class GPSMotionCalculator:
    ...
    def __init__(self, config: MotionPredictionConfig):
        ...
        if config.enable_debug_logging:
            self.logger.info(f"  Object distance assumption: {config.default_object_distance_m}m")
            self.logger.info(f"  GPS accuracy thresholds: {config.min_gps_accuracy_m}m - {config.max_gps_accuracy_m}m")
    
    def calculate_vehicle_movement(self, gps_prev: GPSData, gps_current: GPSData) -> VehicleMovement:
        """
        ...
        if time_delta <= 0:
            ...
            self.logger.debug(f"Zero time delta detected, using estimated: {estimated_time_delta}s")
        ...
        if self.config.enable_debug_logging and is_valid:
            self.logger.debug(f"Vehicle movement: {distance_moved:.2f}m, "
                            f"Speed: {speed_ms*3.6:.1f} km/h, "
                            f"Heading change: {heading_change:.1f}°")
        
        return movement
    
    def _calculate_translation_meters(self, gps_prev: GPSData, gps_current: GPSData) -> Tuple[float, float]:
        """
    ...
    def _calculate_heading_change(self, gps_prev: GPSData, gps_current: GPSData) -> float:
    ...
    def _validate_movement(self, distance_moved: float, speed_ms: float, gps_accuracy: float) -> bool:
        """Validate if calculated movement is trustworthy"""
        
        # Check GPS accuracy
        if gps_accuracy > self.config.max_gps_accuracy_m:
            if self.config.enable_debug_logging:
                self.logger.debug(f"Movement invalid: GPS accuracy too low ({gps_accuracy:.1f}m)")
            return False
        ...
        if speed_ms > 55.0:
            if self.config.enable_debug_logging:
                self.logger.debug(f"Movement invalid: Speed too high ({speed_ms*3.6:.1f} km/h)")
            return False
...
class ScreenDisplacementPredictor:
    ...
    def __init__(self, config: MotionPredictionConfig):
        ...
        if config.enable_debug_logging:
            self.logger.info(f"  Camera FOV: {config.horizontal_fov_degrees}° x {config.vertical_fov_degrees}°")
            self.logger.info(f"  Pixels per radian: H={self.pixels_per_rad_horizontal:.1f}, V={self.pixels_per_rad_vertical:.1f}")
    
    def predict_screen_displacement(self, vehicle_movement: VehicleMovement, 
                                   object_distance_m: Optional[float] = None) -> ScreenDisplacement:
        """
        ...
        angular_displacement_x, angular_displacement_y = self._calculate_angular_displacement(
            vehicle_movement, object_distance_m
        )
        ...
        confidence = self._calculate_prediction_confidence(vehicle_movement, displacement_x_px, displacement_y_px)
        ...
        if self.config.enable_debug_logging and confidence > 0.5:
            self.logger.debug(f"Screen prediction: ({displacement_x_px:.1f}, {displacement_y_px:.1f})px, "
    ...
    def _calculate_angular_displacement(self, vehicle_movement: VehicleMovement, 
                                      object_distance_m: float) -> Tuple[float, float]:
        """
    ...
    def _calculate_prediction_confidence(self, vehicle_movement: VehicleMovement,
                                       displacement_x_px: float, displacement_y_px: float) -> float:
        """Calculate confidence in the prediction"""
        
        # Base confidence from GPS accuracy
        accuracy_confidence = 1.0 - min(1.0, vehicle_movement.gps_accuracy_m / self.config.max_gps_accuracy_m)
        
        # Movement confidence (very small movements are less reliable)
        movement_confidence = min(1.0, vehicle_movement.distance_moved_m / 1.0)  # Confident above 1m movement
        ...
        # Combined confidence
        combined_confidence = accuracy_confidence * movement_confidence * displacement_confidence
        
        return max(0.0, min(1.0, combined_confidence))
    
    def _is_prediction_reasonable(self, displacement_x_px: float, displacement_y_px: float) -> bool:
...
class MotionPredictor:
    ...
    def predict_object_positions(self, gps_prev: GPSData, gps_current: GPSData,
                               current_track_positions: Dict[int, np.ndarray]) -> Dict[int, Dict]:
        """
        ...
        vehicle_movement = self.gps_calculator.calculate_vehicle_movement(gps_prev, gps_current)
        ...
        screen_displacement = self.screen_predictor.predict_screen_displacement(vehicle_movement)
        ...
        for track_id, last_position in current_track_positions.items():
            ...
            predictions[track_id] = {
                ...
                'displacement': np.array([screen_displacement.displacement_x_px, 
        ...
        if self.config.enable_debug_logging and predictions:
            self.logger.debug(f"Generated {len(predictions)} position predictions with "
...
def create_motion_prediction_config(object_distance_m: float = 30.0,
                                   gps_accuracy_threshold_m: float = 3.0,
                                   prediction_tolerance_px: float = 15.0,
                                   enable_debug: bool = True) -> MotionPredictionConfig:
    """
